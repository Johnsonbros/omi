...
- **Memory Curation System**: Automates classification, tagging, and enrichment of memories. Includes a mobile-first swipe-based curation interface (Tinder-like) where users swipe left to save and right to remove memories. Rejected memories prompt for feedback (reason selection + optional details) to create a reinforcement learning loop for improving future curation quality.
